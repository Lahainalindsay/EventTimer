...
type EventData = { id:string; name:string; date:string; venue:string; segments:Segment[]; active:number; remaining:number; running:boolean; message:string; updatedAt:number };
...
const showTime=(s:number)=>`${s<0?"+":""}${pad(Math.floor(Math.abs(s)/60))}:${pad(Math.abs(s)%60)}`;
...
const authErrorMessage=(reason:unknown)=>{
  ...
  if(message.toLowerCase().includes("email rate limit")){
    return "Runline’s confirmation-email service is at its Supabase sending limit. Retrying immediately will not work. Custom production email delivery must be configured before signup can continue.";
  }
  return message;
};
...
export default function RunlineApp(){
  ...
  if(!ready)return <main className="loading-state">Connecting Runline securely…</main>;
  ...
  const submit=async(ev:FormEvent<HTMLFormElement>)=>{
    ...
    try{
      if(mode==="signup"){
        ...
        setNotice("Password updated. You can continue to Runline.");
      }
    }catch(reason){setError(authErrorMessage(reason))}
    finally{setBusy(false)}
  };
  return <main className="auth-shell"><section className="auth-card">
    <div className="auth-brand"><span className="brand-mark"><span/></span><strong>Runline</strong></div>
    <p className="eyebrow">LIVE EVENT CONTROL</p>
    <h1>{mode==="signup"?"Create your Runline account":mode==="reset"?"Reset your password":mode==="update"?"Choose a new password":"Sign in to Runline"}</h1>
    ...
    <small>Connected to Runline Production</small>
  </section></main>;
}
...
function EventFlowTimer({session,accountOnly}:{session:Session;accountOnly:boolean}){
  ...
  const mapRuntime=(event:EventData,runtime:RuntimeRow|null|undefined)=>{
    ...
    let remaining=runtime.duration_seconds+runtime.manual_offset_seconds;
    if(runtime.timer_status==="running"&&runtime.started_at)remaining-=Math.floor((Date.now()-new Date(runtime.started_at).getTime())/1000);
    return {...event,active,remaining,running:runtime.timer_status==="running",updatedAt:new Date(runtime.updated_at).getTime()};
  };
  ...
  const loadCloud=useCallback(async()=>{
    ...
    const mapped=(eventRows??[]).map(row=>{
      ...
      const base:EventData={id:row.id,name:row.name,date:row.event_date||"",venue:row.venue||"Main Stage",segments,active:0,remaining:(segments[0]?.duration??10)*60,running:false,message:(messages??[]).find(m=>m.event_id===row.id)?.body??"",updatedAt:new Date(row.updated_at).getTime()};
  ...
  useEffect(()=>{
    if(!currentId)return;
    const channel=supabase.channel(`runline-event-${currentId}`)
  ...
  useEffect(()=>{if(!current?.running)return;const id=window.setInterval(()=>setEvents(all=>all.map(e=>e.id===currentId?{...e,remaining:e.remaining-1}:e)),1000);return()=>clearInterval(id)},[current?.running,currentId]);
  ...
  const persistRuntime=async(e:EventData)=>{
    ...
    const {error}=await supabase.from("event_runtime").upsert({event_id:e.id,current_agenda_item_id:active?.id??null,timer_mode:"countdown",timer_status:e.running?"running":"paused",duration_seconds:e.remaining,started_at:e.running?new Date().toISOString():null,paused_at:e.running?null:new Date().toISOString(),accumulated_paused_seconds:0,manual_offset_seconds:0,updated_by:session.user.id,updated_at:new Date().toISOString()},{onConflict:"event_id"});
  ...
  const setTimer=(seconds:number,running=current?.running??false)=>commitRuntime(e=>({...e,remaining:seconds,running,updatedAt:Date.now()}));
  const jump=(index:number,run=true)=>{if(!current?.segments[index])return;commitRuntime(e=>({...e,active:index,remaining:e.segments[index].duration*60,running:run,updatedAt:Date.now()}))};
  const toggleTimer=()=>commitRuntime(e=>({...e,running:!e.running,updatedAt:Date.now()}));
  ...
  const saveEvent=async(ev:FormEvent<HTMLFormElement>)=>{
    ...
    const fresh:EventData={id:created.id,name,date,venue,segments,active:0,remaining:segments[0].duration*60,running:false,message:"",updatedAt:Date.now()};
    setEvents(a=>[...a,fresh]);setCurrentId(fresh.id);setCreateOpen(false);setScreen("live");await persistRuntime(fresh);setFeedback("Event saved to Runline cloud");
  };
  ...
  const sendMessage=async(body:string)=>{if(!current)return;const text=body.trim();if(!text)return;const {error}=await supabase.from("event_messages").insert({event_id:current.id,body:text,priority:"normal",created_by:session.user.id});if(error){setFeedback(`Message failed: ${error.message}`);return}updateCurrent({...current,message:text});setDraftMessage("");setFeedback("Message sent through Runline cloud")};
  ...
  if(!hydrated)return <main className="loading-state">Loading your Runline events…</main>;
  if(displayMode&&current&&segment)return <main className={`display-view ${statusClass}`}><button className="exit-display" onClick={()=>{location.href="/dashboard"}}>Exit display</button><div className="display-kicker"><Radio size={16}/> {connection==="live"?"LIVE CLOUD":"NOT SYNCHRONIZED"} · {current.venue.toUpperCase()}</div><div className="display-title">{segment.title}</div><div className="display-clock" aria-live="polite">{showTime(current.remaining)}</div><div className="display-status">{connection==="offline"?"OFFLINE":current.running?status:"PAUSED"}</div>{current.message&&<div className="display-message">{current.message}</div>}<div className="display-next">NEXT&nbsp;&nbsp; {next?.title??"End of show"}</div></main>;

  return <main className="app-shell">
    <header className="topbar"><button className="brand" onClick={()=>setScreen(current?"live":"events")}><span className="brand-mark"><span/></span><strong>Runline</strong><em>LIVE</em></button><div className="event-switcher"><span>{current?.name??"No event selected"}</span><small>{current?`${current.date} · ${current.venue}`:"Create an event to begin"}</small></div><div className="top-actions"><span className={`sync ${connection}`}>{connection==="offline"?<WifiOff size={14}/>:<Wifi size={14}/>} {connection==="live"?"Cloud live":connection==="offline"?"Offline":"Reconnecting"}</span><button className="icon-button" disabled title="Global settings are not available in this release" aria-label="Settings unavailable"><Settings2 size={18}/></button><button className="avatar" onClick={()=>setScreen("account")} title="Account">{(session.user.email?.[0]??"R").toUpperCase()}</button></div></header>
    ...
    <section className="workspace" id="top">
      ...
      {screen==="live"&&current&&segment&&<>
        ...
        <div className="status-strip"><div><span>EVENT STATUS</span><strong className="green"><Circle size={9} fill="currentColor"/> {current.running?"Running":"Ready / paused"}</strong></div><div><span>DATE</span><strong>{current.date}</strong></div><div><span>PROJECTED FINISH</span><strong>{projected}</strong></div><div><span>STORAGE</span><strong>Runline cloud</strong></div><div className="connection-label"><span className={connection==="live"?"pulse":"offline"}/>{connection==="live"?"Realtime connected":connection==="offline"?"Offline — state may be stale":"Reconnecting"}</div></div>
        <div className="console-grid"><section className={`timer-card ${statusClass}`}><div className="timer-meta"><span><Radio size={14}/> {current.running?"NOW LIVE":"READY"}</span><button disabled title="Countdown mode only in this release">COUNTDOWN</button></div><div className="segment-title">{segment.title}</div><div className="speaker">{segment.person}</div><div className="timer" aria-live="polite">{showTime(current.remaining)}</div><div className="timer-state"><span>{current.running?status:"PAUSED"}</span><small>Warning at 02:00 · Urgent at 00:30</small></div>
