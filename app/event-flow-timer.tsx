...
import { adjustTime, computeRemainingSeconds, formatTime, pause as pauseTimer, resume as resumeTimer } from "@/lib/timer-engine";
...
function EventFlowTimer({session,accountOnly}:{session:Session;accountOnly:boolean}){
  ...
  const adjustTimer=(deltaSeconds:number)=>{const now=Date.now();commitRuntime(e=>{const ts={durationSeconds:e.timerDuration,manualOffsetSeconds:0,status:e.running?"running" as const:"paused" as const,startedAt:e.timerStartedAt};const next=adjustTime(ts,deltaSeconds,now);const remaining=computeRemainingSeconds(next,now);return {...e,remaining,timerDuration:next.durationSeconds,timerStartedAt:next.startedAt,updatedAt:now}})};
